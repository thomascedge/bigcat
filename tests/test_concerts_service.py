...
class TestConcertsService:
    ...
    def test_search_concerts(self, test_concert, db_session):
        ...
        assert len(concerts.concert_list) > 0
        assert any(concert.concert_id == query['concert_id'] for concert in concerts.concert_list)

        concerts = concerts_service.search_concerts(concert_id=query['concert_id'], db=db_session)
        assert len(concerts.concert_list) > 0
        assert any(concert.concert_id == query['concert_id'] for concert in concerts.concert_list)
    
    def test_create_concert(self, test_user, test_concert, db_session):
        concert = concerts_service.create_concert(test_user, test_concert, db_session)
        assert concert.concert_id == test_concert.concert_id

        with pytest.raises(ConcertCreationError):
            concerts_service.create_concert(test_user, None, db_session)

    def test_update_concert(self, test_user, test_concert, db_session):
        db_session['concert'].insert_one(test_concert.model_dump())
        ...
        updated_concert = concerts_service.update_concert(test_user, test_concert.concert_id, concert_update, db_session)
    ...
    def test_cancel_concert(self, test_user, test_concert, db_session):
        db_session['concert'].insert_one(test_concert.model_dump())
        concerts_service.cancel_concert(test_user, test_concert.concert_id, db_session)
